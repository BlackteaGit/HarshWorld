...
namespace HarshWorld
{
    public class HarshWorld_Patches : IWTFMod
	{
		...
		public class FreelancerFactionRev2_updateTraders //calculating demand for tradegoods
		{
			...
			[HarmonyPrefix]
			private static Boolean Prefix(FreelancerFactionRev2 __instance, float elapsed)
			{

				bool flag = __instance.traders.Count > 0;
				checked
				{
					if (flag)
					{
						ulong num = __instance.traders.Dequeue();
						bool flag2 = __instance.ships.ContainsKey(num);
						if (flag2)
						{
							__instance.traders.Enqueue(num);
							bool flag3 = !__instance.jobs.ContainsKey(num);
							if (flag3)
							{
								NonPlayerShip ship = __instance.ships[num];
								var args = new object[] { ship };
								var flags = BindingFlags.Public | BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Static;
								bool flag4 = !(Boolean)typeof(FreelancerFactionRev2).GetMethod("findTradeRoute", flags, null, new Type[] { typeof(NonPlayerShip) }, null).Invoke(__instance, args); // using reflection to call a private method findTradeRoute
								if (flag4)
								{
									bool flag5 = !(Boolean)typeof(FreelancerFactionRev2).GetMethod("findTradeRoute", flags, null, new Type[] { typeof(NonPlayerShip) }, null).Invoke(__instance, args);
									if (flag5)
									{
										bool flag6 = !(Boolean)typeof(FreelancerFactionRev2).GetMethod("findTradeRoute", flags, null, new Type[] { typeof(NonPlayerShip) }, null).Invoke(__instance, args);
										if (flag6)
										{
											bool flag7 = !(Boolean)typeof(FreelancerFactionRev2).GetMethod("findTradeRoute", flags, null, new Type[] { typeof(NonPlayerShip) }, null).Invoke(__instance, args);
											if (flag7)
											{
												bool flag8 = !(Boolean)typeof(FreelancerFactionRev2).GetMethod("findTradeRoute", flags, null, new Type[] { typeof(NonPlayerShip) }, null).Invoke(__instance, args);
												if (flag8)
												{
												}
											}
							...
							{
								TradeJob tradeJob = __instance.jobs[num];
								NonPlayerShip nonPlayerShip = __instance.ships[num];
								bool collected = tradeJob.collected;
								if (collected)
								{
									float num2 = tradeJob.destination.offerPrices[tradeJob.product];
									bool flag9 = num2 < tradeJob.buyPrice;
									if (flag9)
									{
										float num3 = EconomyNode.distance(tradeJob.destination, nonPlayerShip.position, nonPlayerShip.grid);
										EconomyNode economyNode = tradeJob.destination;
										List<EconomyNode> list = __instance.world.economy.demand[tradeJob.product];
										for (int i = 0; i < list.Count; i++)
										{
											var economyNode2 = list[i];
											float num4 = economyNode2.offerPrices[tradeJob.product];
											int num5 = economyNode2.capacity[tradeJob.product] - (int)economyNode2.storage[tradeJob.product];
											bool flag10 = num4 > num2;
											if (flag10)
											{
												float num6 = EconomyNode.distance(economyNode2, nonPlayerShip.position, nonPlayerShip.grid);
												bool flag11 = unchecked(num6 <= num3 + 16f && (double)num5 > (double)nonPlayerShip.countInventory(tradeJob.product) * 1.2);
												if (flag11)
												{
													...
													{
													num.ToString(),
													" found a better place to sell ",
													tradeJob.product.ToString(),
													". old dest: ",
													tradeJob.destination.ToString(),
													" new dest: ",
													economyNode.ToString()
													}));
												}
											}
										}
										...
										__instance.jobs[num] = tradeJob;
									}
									else
									{
										bool isDocked = nonPlayerShip.isDocked;
										if (isDocked)
										{
											bool flag12 = nonPlayerShip.trySellAll(tradeJob.destination, tradeJob.product);
											if (flag12)
											{
												Debug.WriteLine(string.Concat(new string[]
												{
												...
												__instance.jobs.Remove(num);
											}
											else
											{
												int num7 = 0;
												bool flag13 = __instance.searchCounts.ContainsKey(num);
												if (flag13)
												{
													num7 = __instance.searchCounts[num];
												}
												num7++;
												bool flag14 = num7 > 3;
												if (flag14)
												{
													try
													{
														int num8 = nonPlayerShip.countInventory(tradeJob.product);
														List<EconomyNode> list2 = __instance.world.economy.demand[tradeJob.product];
														EconomyNode economyNode3 = null;
														float num9 = -1f;
														for (int i = 0; i < list2.Count; i++)
														{
															var economyNode4 = list2[i];
															int num10 = economyNode4.capacity[tradeJob.product] - (int)economyNode4.storage[tradeJob.product];
															bool flag15 = num10 >= num8;
															if (flag15)
															{
																float num11 = EconomyNode.distance(economyNode4, nonPlayerShip.position, nonPlayerShip.grid);
																bool flag16 = num11 <= num9 || num9 < 0f;
																if (flag16)
																{
														...
														if (flag17)
														{
															...
															{
															num.ToString(),
															" gave up trying to sell ",
															tradeJob.product.ToString(),
															". rerouting to ",
															economyNode3.ToString()
															}));
														}
														else
														{
												...
												if (flag18)
												{
													try
													{
														Debug.WriteLine(num.ToString() + " deleted " + tradeJob.product.ToString() + " from frustration because it could not be sold");
													...
													{
														Debug.WriteLine(num.ToString() + " was despawned due to an exception while attempting to delete cargo");
														__instance.despawnShip(num);
													}
												}
											}
										}
								...
								{
									float num12 = tradeJob.source.salesPrices[tradeJob.product];
									bool flag19 = num12 > tradeJob.buyPrice;
									if (flag19)
									{
										Debug.WriteLine(num.ToString() + " abandoned job before buying product because the price changed");
										nonPlayerShip.moveToLocation(nonPlayerShip.position + RANDOM.squareVector(500f), nonPlayerShip.grid);
										__instance.jobs.Remove(num);
									}
									else
									{
										bool isDocked2 = nonPlayerShip.isDocked;
										if (isDocked2)
										{
											bool flag20 = nonPlayerShip.tryBuyBudget(tradeJob.source, tradeJob.product);
											if (flag20)
											{
												Debug.WriteLine(string.Concat(new string[]
												{
											...
											{
												Debug.WriteLine(num.ToString() + " failed to purchase " + tradeJob.product.ToString());
												nonPlayerShip.moveToLocation(nonPlayerShip.position + RANDOM.squareVector(500f), nonPlayerShip.grid);
												__instance.jobs.Remove(num);
						...
						{
							Debug.WriteLine(num.ToString() + "Trade ship was removed because it didn't exist in ships struct");
						}
					}
				}
